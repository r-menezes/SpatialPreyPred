...
class Environment():
    def __init__(self, Nagents, inter_distance, inter_rate, birth_rate, death_rate, centers, dispersal_kernel, noise=None, data_interval=50, h=1e-3, env_size=1e2, seed=34567, rng=None, id=None, transient_steps=1000, check_termination_N_data=40, **kwargs):
        ...
        # interaction-related
        self.positions_tree = KDTree(self.centers, compact_nodes=False, balanced_tree=False, boxsize=self.env_size)
        ...
        while self.steps < max_steps:
            ...
            if self.steps % self.data_interval == 0:
                ...
                if self.steps > self.transient_steps:
                    test, testValue = self.stop_condition()
                    if test:
                        return self.on_exit(cause='equilibrium', testValue=testValue)
    ...
    def on_exit(self, cause = None, **argv):
        ...
        PCFxs, PCFys = self.calculate_PCF('centers')
        try:
    ...
    def store_data(self):
        self.data = self.data.append({'t': self.t, 'events': self.steps, 'N': self.N, 'births': self.births, 'deaths': self.deaths}, ignore_index=True) # , 'sigma': self.g_sqrts, 'rep': self.id, 'tau': self.taus, 'inter': self.interaction_distance, 'interRate':self.total_interaction_rate, 'birthRate': self.total_birth_rate, 
    ...
    def plot_PCF(self, max_dist, resolution, distances='centers'):
...
def run_environment(dic):
    ...
    env = environment(Nagents=100, inter_distance=dic['inter'], birth_rate=1., inter_rate=1., taus=dic['taus'], g_sqrts=dic['sigma'], centers='random', dispersal_kernel='random', data_interval=50, env_size=1., id=dic['rep'])
    env.run(int(1e4))
